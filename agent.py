...
class MyRLAgent(AgentBrain):
    ...
    def remove_unreachable(self,neighbors):
        keep = []
        for (row_neigh,col_neigh) in neighbors:
            moves1 = self.get_possible_moves((row_neigh,col_neigh))
            keep_moves1 = []
            for x in moves1:
                if x in neighbors:
                    if x not in keep:
                        keep.append(x)
                    keep_moves1.append(x)
            for (row1, col1) in keep_moves1:
                moves2 = self.get_possible_moves((row1,col1))
                for cell in moves2:
                    if cell in neighbors and cell not in keep:
                        keep.append(cell)
        print "after remove_unreachable: "+str(keep)
        return keep
    ...
    def get_possible_moves(self,(row,col)):
        ...
        for (r,c) in moves:
            if r<8 and r>=0 and c<8 and c>=0:
                if ((row,col), (r,c)) in get_environment().maze.walls or ((r,c),(row,col)) in get_environment().maze.walls: 
                    has_wall.append(((row,col), (r,c)))        
                else:
                    keep.append((r,c)) #valid and no wall
        #print "R: "+str(row) + "C: "+str(col)+ " has possible moves of: "+str(keep)
        return keep
    
    def get_neighbors(self, row_of_approximator, col_of_approximator, observations):
        """
        ...
        for num in range(-1,2):
            for num2 in range(-1,2):
                ...
                if new_row>=0 and new_row<8 and new_col>=0 and new_col<8: #valid 
                    if ((row_of_approximator,col_of_approximator), (new_row,new_col)) in get_environment().maze.walls or ((new_row,new_col),(row_of_approximator,col_of_approximator)) in get_environment().maze.walls: 
                        has_wall.append(((row_of_approximator,col_of_approximator), (new_row,new_col)))
                        print "NOT ADDING NEW ROW: "+str(new_row)+" NEW COL: "+str(new_col)
                    else:    
                        neighbors.append((new_row,new_col)) #valid and no wall
        print "walls: "+str(has_wall)
        #print "adding place im in: "+ str((row,col))
        #neighbors.append((row,col))
        print "Unsorted neighbors: " + str(neighbors) 
        neighbors=self.remove_unreachable(neighbors)
    ...
    def value_approximation(self,observations, action):
        ...
        print "I am in: "+str((row_of_approx,col_of_approx))
