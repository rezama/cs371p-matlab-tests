// ------------------------------
// projects/matlab/TestMatlab.c++
// Copyright (C) 2012
// Glenn P. Downing
// ------------------------------

/**
 * To test the program:
 *     g++ -ansi -pedantic -lcppunit -ldl -Wall TestMatlab.c++ -o TestMatlab.app
 *     valgrind TestMatlab.app >& TestMatlab.out
 */

// --------
// includes
// --------

#include "cppunit/extensions/HelperMacros.h" // CPPUNIT_TEST, CPPUNIT_TEST_SUITE, CPPUNIT_TEST_SUITE_END
#include "cppunit/TestFixture.h"             // TestFixture
#include "cppunit/TextTestRunner.h"          // TestRunner

#include "Matrix.h"
#include "Matlab.h"

// ----------
// TestMatlab
// ----------

struct TestMatlab : CppUnit::TestFixture {
  // --------
  // test_cat
  // --------

  void test_horzcat () 
  {
    Matrix<int> x;
    Matrix<int> y;
    Matrix<int> z;
    x = horzcat(x, y);
    CPPUNIT_ASSERT(x.eq(z));
  }

  void test_horzcat_two () {
		Matrix<int> x(2, 2);
		Matrix<int> y(2, 3);
		Matrix<int> z(2, 5);
		x = horzcat(x, y);
		CPPUNIT_ASSERT(x.eq(z));
	}

  void test_vertcat () 
  {
    Matrix<int> x;
    Matrix<int> y;
    Matrix<int> z;
    x = vertcat(x, y);
    CPPUNIT_ASSERT(x.eq(z));
  }

  void test_vertcat_two () {
		Matrix<int> x(3, 2);
		Matrix<int> y(5, 2);
		Matrix<int> z(8, 2);
		x = vertcat(x, y);
		CPPUNIT_ASSERT(x.eq(z));
	}

  // ---------
  // test_diag
  // ---------

  void test_diag () 
  {
    Matrix<int> x;
    Matrix<int> y;
    x = diag(x);
    CPPUNIT_ASSERT(x.eq(y));
  }

  void test_diag_two () {
		Matrix<int> x(2, 2, 1);
		Matrix<int> y(2, 2, 0);
    y[0][0] = 1;
    y[1][1] = 1;
		x = diag(x);
		CPPUNIT_ASSERT(x.eq(y));
	}

  // --------
  // test_dot
  // --------

  void test_dot () 
  {
    Matrix<int> x;
    Matrix<int> y;
    Matrix<int> z;
    x = dot(x, y);
    CPPUNIT_ASSERT(x.eq(z));
  }

	void test_dot_two () {
		Matrix<int> A(3, 0);
		Matrix<int> B(3, 0);
		Matrix<int> ans;
		A = dot(A, B);
		CPPUNIT_ASSERT(A.eq(ans));
	}

	void test_dot_three () {
		Matrix<int> A(3, 0);
		Matrix<int> B(0, 0);
		Matrix<int> ans;
		A = dot(A, B);
		CPPUNIT_ASSERT(A.eq(ans));
	}

	void test_dot_four () {
		Matrix<int> A(1, 1, 3);
		Matrix<int> B(1, 1, 5);
		Matrix<int> ans(1, 1, 15);
		A = dot(A, B);
		CPPUNIT_ASSERT(A.eq(ans));
	}

	void test_dot_five () {
		Matrix<int> A(1, 2, 3);
		Matrix<int> B(2, 1, 5);
		Matrix<int> ans(1, 1, 30);
		A = dot(A, B);
		CPPUNIT_ASSERT(A.eq(ans));
	}

	void test_dot_six () {
		Matrix<int> A(2, 1, 3);
		Matrix<int> B(1, 2, 5);
		Matrix<int> ans(2, 2, 15);
		A = dot(A, B);
		CPPUNIT_ASSERT(A.eq(ans));
	}

	void test_dot_seven () {
		Matrix<int> A(2, 2, 3);
		Matrix<int> B = horzcat(Matrix<int>(2, 1, 5), Matrix<int>(2, 1, 0));
		Matrix<int> ans = horzcat(Matrix<int>(2, 1, 30), Matrix<int>(2, 1, 0));
		A = dot(A, B);
		CPPUNIT_ASSERT(A.eq(ans));
	}

  // --------
  // test_eye
  // --------

  void test_eye () 
  {
    Matrix<int> x;
		Matrix<int> y(2, 3);
		y[0][0] = 1;
		y[1][1] = 1;
		x = eye< Matrix<int> >(2, 3);
		CPPUNIT_ASSERT(x.eq(y));
  }

  // -------------
  // test_linsolve
  // -------------

  void test_linsolve () 
  {
    Matrix<int> x;
    Matrix<int> y;
    Matrix<int> z;
    x = linsolve(x, y);
    CPPUNIT_ASSERT(x.eq(z));
  }

  void test_linsolve_two () {
		Matrix<int> A(3, 0);
		Matrix<int> B(3, 0);
		Matrix<int> ans;
		A = linsolve(A, B);
		CPPUNIT_ASSERT(A.eq(ans));
	}
	
	void test_linsolve_three () {
		Matrix<int> A(3, 0);
		Matrix<int> B(3, 5);
		Matrix<int> ans;
		A = linsolve(A, B);
		CPPUNIT_ASSERT(A.eq(ans));
	}
	
	void test_linsolve_four () {
		const int m = 1;
		const int n = 1;
		const int k = 1;
		Matrix<int> A(m, n, 2);
		Matrix<int> B(m, k, 6);
		Matrix<int> ans(n, k, 3);
		A = linsolve(A, B);
		//CPPUNIT_ASSERT(A.eq(ans));
	}
	
	void test_linsolve_five () {
		const int m = 1;
		const int n = 2;
		const int k = 1;
		Matrix<int> A(m, n, 2);
		Matrix<int> B(m, k, 16);
		Matrix<int> ans(n, k, 8);
		A = linsolve(A, B);
		//CPPUNIT_ASSERT(A.eq(ans));
	}

  // ---------
  // test_ones
  // ---------

  void test_ones () 
  {
    Matrix<int> x;
		Matrix<int> y(2, 3, 1);
		x = ones< Matrix<int> >(2, 3);
		CPPUNIT_ASSERT(x.eq(y));
  }

  // ---------
  // test_rand
  // ---------

  void test_rand () 
  {
    Matrix<int> x;
		Matrix<int> y;
		x = rand< Matrix<int> >(2, 3);
		CPPUNIT_ASSERT(x.size() == 2);
		CPPUNIT_ASSERT(x[0].size() == 3);
  }

  void test_rand_two () {
		Matrix<int> x;
		Matrix<int> y;
		x = rand< Matrix<int> >(2, 3);
		
		CPPUNIT_ASSERT(x.size() == 2);
		CPPUNIT_ASSERT(x[0].size() == 3);
	}

  // --------------
  // test_transpose
  // --------------

  void test_transpose () 
  {
    Matrix<int> x;
    Matrix<int> y;
    x = transpose(x);
    CPPUNIT_ASSERT(x.eq(y));
  }

  void test_transpose_two () {
		Matrix<int> x(2, 3);
		Matrix<int> y(3, 2);
		x = transpose(x);
		CPPUNIT_ASSERT(x.eq(y));
	}

  // ---------
  // test_tril
  // ---------

  void test_tril () 
  {
    Matrix<int> x;
    Matrix<int> y;
    x = tril(x);
    CPPUNIT_ASSERT(x.eq(y));
  }

  void test_tril_two () {
		Matrix<int> x(2, 2, 1);
		Matrix<int> y(2, 2, 1);
		y[0][1] = 0;
		x = tril(x);
		
		CPPUNIT_ASSERT(x.eq(y));
	}

  // ---------
  // test_triu
  // ---------

  void test_triu () 
  {
    Matrix<int> x;
    Matrix<int> y;
    x = triu(x);
    CPPUNIT_ASSERT(x.eq(y));
  }

  void test_triu_two () {
		Matrix<int> x(2, 2, 1);
		Matrix<int> y(2, 2, 1);
		y[1][0] = 0;
		x = triu(x);
		
		CPPUNIT_ASSERT(x.eq(y));
	}

  // ----------
  // test_zeros
  // ----------

  void test_zeros () 
  {
    Matrix<int> x;
    Matrix<int> y(2, 3, 0);
    x = zeros< Matrix<int> >(2, 3);
    CPPUNIT_ASSERT(x.eq(y));
  }

  // -----
  // suite
  // -----

  CPPUNIT_TEST_SUITE(TestMatlab);
  CPPUNIT_TEST(test_horzcat);
  CPPUNIT_TEST(test_horzcat_two);
  CPPUNIT_TEST(test_vertcat);
  CPPUNIT_TEST(test_vertcat_two);
  CPPUNIT_TEST(test_diag);
  CPPUNIT_TEST(test_diag_two);
  CPPUNIT_TEST(test_dot);
  CPPUNIT_TEST(test_dot_two);
  CPPUNIT_TEST(test_dot_three);
  CPPUNIT_TEST(test_dot_four);
  CPPUNIT_TEST(test_dot_five);
  CPPUNIT_TEST(test_dot_six);
  CPPUNIT_TEST(test_dot_seven);
  CPPUNIT_TEST(test_eye);
  CPPUNIT_TEST(test_linsolve);
  CPPUNIT_TEST(test_linsolve_two);
  CPPUNIT_TEST(test_linsolve_three);
  CPPUNIT_TEST(test_linsolve_four);
  CPPUNIT_TEST(test_linsolve_five);
  CPPUNIT_TEST(test_ones);
  CPPUNIT_TEST(test_rand);
  CPPUNIT_TEST(test_rand_two);
  CPPUNIT_TEST(test_transpose);
  CPPUNIT_TEST(test_transpose_two);
  CPPUNIT_TEST(test_tril);
  CPPUNIT_TEST(test_tril_two);
  CPPUNIT_TEST(test_triu);
  CPPUNIT_TEST(test_triu_two);
  CPPUNIT_TEST(test_zeros);
  CPPUNIT_TEST_SUITE_END();
};

// ----
// main
// ----

int main () 
{
  using namespace std;
  ios_base::sync_with_stdio(false); // turn off synchronization with C I/O
  cout << "TestMatlab.c++" << endl;

  CppUnit::TextTestRunner tr;
  tr.addTest(TestMatlab::suite());
  tr.run();

  cout << "Done." << endl;
  return 0;
}
